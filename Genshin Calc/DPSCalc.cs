...
namespace Genshin_Calc
{
    public partial class DPSCalc : Form
    {
        ...
        double Skill, TimeCount, OtherBuff, PhysBuff, ElemBuff, PhysRES, ElemRES;
        double basic;
        double[] Elem, Phys;
        ...
        public void Input(double 技能倍率, double 其他增伤, double 其他, double 反应增伤, int 元素精通, int 等级)
        {
            Skill = 技能倍率;
            basic = 其他;
            OtherBuff = 其他增伤;
        ...
        {
            ElemRES = Convert.ToDouble(Anemo_RES.Text);
            PhysRES = Convert.ToDouble(Phys_RES.Text);
...
//Elem_Basic = basic * ElemRES ;
//Phys_Basic = basic * PhysRES ;
//foreach (int i in Elem)//单段元素伤害=DMG_E=basic*ElemBuff*ElemRES
//{
//    Elem[i] += Elem_Basic;
//}
//foreach (int i in Phys)//单段物理伤害=DMG_P=basic*PhysBuff*PhysRES
//{
//    Phys[i] += Phys_Basic;
//}
...
//foreach (DataGridViewRow row in CalcTable.Rows)
//{
//    if (Cel(row, 0) * Cel(row, 1) != 0)
//    {
//        //Cell4=Basic * (ElemBuff + OtherBuff)*Cell1*Cell2(if Cell3.SelectedIndex==0&&Cell4.SelectedIndex==0)
//        //Cell4=Basic * (PhysBuff + OtherBuff)*Cell1*Cell2(if Cell3.SelectedIndex==1&&Cell4.SelectedIndex==0)
//        row.Cells[4].Value = 1.990;
//        if (row.Cells[2].Value.ToString() == "物理伤害")
//        {
//            //Cell4=Basic * (PhysBuff + OtherBuff)*Cell1*Cell2(if Cell3.SelectedIndex==1&&Cell4.SelectedIndex==0)
//            row.Cells[3].Value = "无反应";
//            row.Cells[3].ReadOnly = true;
//            CalcTable.Rows[CalcTable.CurrentRow.Index].Cells[4].Value = /*Phys_Basic * (PhysBuff + OtherBuff) */ Tablex(1,1) / 100 * Tablex(1, 2);
//        }
//        else
//        {
